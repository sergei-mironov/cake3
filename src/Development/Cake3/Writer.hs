...
multyTarget :: (Monoid x) => Map Target (RecipeT x) -> Map Target (RecipeT x)
multyTarget m = runTmp (traverseMap f m) where
...
toMake (vs_, rs_, p) = 
  let (vs,rs) = (map snd $ M.toList vs_, applyPlacement (multyTarget rs_) p)
